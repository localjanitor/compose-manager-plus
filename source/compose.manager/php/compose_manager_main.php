...
foreach ($composeProjects as $project) {
  ...
  if ( $isup ) {
    if ( $isexited && !$isrunning) {
        $statusIcon = "<i class='fa fa-square stopped red-text'></i>";
    }
    else {
      if ( $isrunning && !$isexited && !$ispaused && !$isrestarting) {
        $statusIcon = "<i class='fa fa-play started green-text'></i>";
      }
      elseif( $ispaused && !$isexited && !$isrunning && !$isrestarting)
      {
        $statusIcon = "<i class='fa fa-pause started orange-text'></i>";
      }
      elseif( $ispaused && !$isexited )
      {
        $statusIcon = "<i class='fa fa-play started orange-text'></i>";
      }
      else
      {
        $statusIcon = "<i class='fa fa-play started red-text'></i>";
      }
    }
  }

  // Build card HTML
  $o .= "<div class='stack-card'>";
  $o .= "<div class='stack-card-header'>";
  $o .= "<span class='ca_nameEdit stack-name' id='name$id' data-nameName='$projectName' data-isup='$isup' data-scriptName=".escapeshellarg($project).">";
  $o .= "<i class='fa fa-gear'></i> <span class='stack-title'>$projectName</span> $statusIcon";
  $o .= "</span>";
  $o .= "<span class='ca_descEdit stack-desc' data-scriptName=".escapeshellarg($project)." id='desc$id' title='$description'>$description</span>";
  $o .= "</div>";

  $o .= "<div class='stack-card-actions'>";

  // Compact icon buttons
  $buttons = [
    ["fa-arrow-up", "ComposeUp", "up", "Compose Up"],
    ["fa-arrow-down", "ComposeDown", "down", "Compose Down"],
    ["fa-refresh", "UpdateStack", "update", "Update Stack"]
  ];

  $o .= "<div class='stack-buttons'>";
  foreach ($buttons as $button)
  {
    if ( $profiles ) {
      $onClickParams = "&quot;$compose_root/$project&quot;";
      $o .= createComboButton("<i class='fa $button[0]'></i>", "$button[2]-$id", $button[1], $onClickParams, $profiles);
    } else {
      $o .= "<button type='button' class='stack-btn $button[2]-button' id='$button[2]-$id' onclick='$button[1](&quot;$compose_root/$project&quot;);' title='$button[3]'><i class='fa $button[0]'></i></button>";
    }
  }
  $o .= "</div>";
...
<span class='tipsterallowed' hidden><input type='button' value='Add New Stack' onclick='addStack();'></span>
</div>
